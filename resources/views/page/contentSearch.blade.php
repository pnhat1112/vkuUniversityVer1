			...
			<ul class="navbar-nav">
				...
				<li class="nav-item dropdown">
					...
					<ul class="dropdown-menu">
						...
						<li><a class="dropdown-item" href="{{URL::to('index')}}"> Bộ phận khác </a></li>
					</ul>
				</li>
			</ul>
		</div> <!-- navbar-collapse.// -->
...
<div class="heading-notifications">
	<h3 style="padding-left: 20px;">Kết quả tìm kiếm cho <span style="color: red; font-weight: 600;">{{$txtSearch}}:</span></h3>
</div>
		...
		<div class="content-left">
			<div class="content-left1">
				<ul>
					...
					<li><a href="notifications/{{$value->slug}}">{{$value->title}} </a><span> - {{ date('d/m/y H:i', strtotime($value->created_at)) }} </span></li>
					@endforeach
					@foreach ($resultSearchKhtc as $value)
					<li><a href="notifications-khtc/{{$value->slug}}">{{$value->title}} </a><span> - {{ date('d/m/y H:i', strtotime($value->created_at)) }} </span></li>
					@endforeach
					@foreach ($resultSearchTtdn as $value)
					<li><a href="notifications-ttdn/{{$value->slug}}">{{$value->title}} </a><span> - {{ date('d/m/y H:i', strtotime($value->created_at)) }} </span></li>
					@endforeach
					@if($resultSearch->isEmpty() && $resultSearchKhtc->isEmpty() && $resultSearchTtdn->isEmpty())
					<h4 style="color: #1578b3; font-weight: 600;">Không có kết quả nào phù hợp</h4>
					@endif
				</ul>
			</div>
		</div>
